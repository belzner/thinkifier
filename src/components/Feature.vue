...
export default {
  ...
  methods: {
    ...
    moving: function (e) {
      ...
      if (newTop >= 0 && newTop <= this.workspaceSize - this.size) {
        this.top = newTop;
      }

      const newLeft = this.startLeft + (e.x - this.startX);
      if (newLeft >= 0 && newLeft <= this.workspaceSize - this.size) {
        this.left = newLeft;
      }
    },
    resizing: function (e) {
      const newSize = this.startSize + Math.min(e.x - this.startX, e.y - this.startY);
      if (newSize >= 20 && newSize + this.top <= this.workspaceSize && newSize + this.left <= this.workspaceSize) {
        this.size = newSize;
      }
    }
  }
